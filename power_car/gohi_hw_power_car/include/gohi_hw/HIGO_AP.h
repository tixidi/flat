        ...
        while (!ack_ready_)
        {
            ...
            {    
                if (hflinkmodbus_->byteAnalysisCall_R(data[i]))
                {
                        ...
                        if(command == READ_MOT5_ERROR_STATE)
                        {
                            
                            std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT5_ERROR_STATE   receive ok"<<std::endl;
                            
                        } 
                        else if(command == READ_MOT6_ERROR_STATE)
                        {
                            std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT6_ERROR_STATE   receive ok"<<std::endl;
                        
                        }  
                        else if(command == READ_MOT5_REAL_POSITION)
                        {
                            std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT5_REAL_POSITION   receive ok"<<std::endl;
                        
                        }else if(command == READ_MOT6_REAL_POSITION)
                        {
                        std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT6_REAL_POSITION   receive ok"<<std::endl;
            ...
            if (cicle_timer_.expires_from_now().is_negative())
            {
                if(command == READ_MOT5_ERROR_STATE)
                {
                    std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT5_ERROR_STATE   Timeout  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
                     
                } 
                else if(command == READ_MOT6_ERROR_STATE)
                {
                    std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT6_ERROR_STATE  Timeout  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
                   
                }  
                else if(command == READ_MOT5_REAL_POSITION)
                {
                    std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT5_REAL_POSITION  Timeout  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
                   
                }else if(command == READ_MOT6_REAL_POSITION)
                {
                   std::cerr<<"the "<< time_out_count<<" count's "<<"READ_MOT6_REAL_POSITION  Timeout  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
    ...
    inline void readCommandModbus1(const MotorModbusCommand &command)
    {
        ...
        while (!ack_ready_)
        {
            ...
            {     
                ...
                {
                    if(command == SET_CAR3_LEFT_SPEED_CONTROL)
                    {
                           
                    std::cerr<<"the "<< time_out_count<<" count's "<<"SET_CAR3_LEFT_SPEED_CONTROL write receive ok  "<<std::endl;
                    }
                    else if(command == SET_CAR3_RIGHT_SPEED_CONTROL)
                    {
                    std::cerr<<"the "<< time_out_count<<" count's "<<"SET_CAR3_RIGHT_SPEED_CONTROL write receive ok  "<<std::endl;
                    }
            ...
            if (cicle_timer_.expires_from_now().is_negative())
            {
                if(command == SET_CAR3_LEFT_SPEED_CONTROL)
                {
                   std::cerr<<"the "<< time_out_count<<" count's "<<"SET_CAR3_LEFT_SPEED_CONTROL write Timeout continue skip this package  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
                }
                else if(command == SET_CAR3_RIGHT_SPEED_CONTROL)
                {
                   std::cerr<<"the "<< time_out_count<<" count's "<<"SET_CAR3_RIGHT_SPEED_CONTROL write Timeout continue skip this package  "<< (float)(ros::Time::now() - currentTime).toSec()<<std::endl;
                }
                ros::Time currentTime = ros::Time::now();
                // time_out_count=0;
